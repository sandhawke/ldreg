...
# $Id: html.py,v 1.3 2009/03/27 04:32:15 sandro Exp $
...
class Streamer:
    ...
    def __str__(self):
        ...
        orig_file = cStringIO.StringIO()
        Writer = codecs.getwriter('utf-8')
        wrapped_file = Writer(orig_file)
        self.writeTo(wrapped_file)
        #return wrapped_file.getvalue()
        return orig_file.getvalue()
...
class Document(Streamer):
   ...
   def writeTo(self, s):
       s.write(u'<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN"\n')
       s.write(u'          "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">')
       self.html.writeTo(s)
...
def xstr(s, inAttr=False):
    """Return the XML-escaped version of s"""
    if not isinstance(s, basestring):
        raise RuntimeError("xstr not given a string", `s`)
    #s = str(s)
    if isinstance(s, str):
        s = s.decode('utf-8')
    s = s.replace("&", "&amp;")
        ...
        for key in keys:
            if self.attrs[key] is not None:
                ...
                if key.endswith("_"):
                    ...
                    s.write(key)
                s.write("=\"")
                s.write(xstr(self.attrs[key], inAttr=True))
                s.write("\"")
        if self.content:
            s.write(">")
...
# $Log: html.py,v $
# Revision 1.3  2009/03/27 04:32:15  sandro
# implemented regression tester, and debugged until it worked
#
# Revision 1.2  2009/03/26 23:56:12  sandro
# Watching Zakim's people-leaving announcements
# Some flailing around some unicode bug; I think I fixed it.
#
# Revision 1.1  2009/03/26 22:00:48  sandro
# copied over from 2007/groupdev, to make it more clear which files
# are needed for CommonScribe
#
# then modified to add:
#    -- getting names from tracker, instead of wiki page
#    -- doing s/foo/bar substitution (in all its forms)
#    -- some meeting conventions used by the xhtml wg (eg agenda:)
#    -- the ivan rule for nicknames taking priority
#    -- a command line interface, when you don't want to use the
#       web-based interface
#    -- configuration for the xhtml-wg
#    -- the GUEST: command (also OBSERVER:) for folks not in the database
#    -- implicit building of "seen" line, in case you don't say "present"
#
